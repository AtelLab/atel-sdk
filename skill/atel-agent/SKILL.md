...
1. 当用户消息是完整的 `atel ` 命令（以 `atel ` 开头）时，必须在终端真实执行该命令（通常是 `cd ~/atel-workspace && atel ...`）。
2. 严禁编造执行结果。禁止在未执行命令时回复类似 `{"status":"success"}`、`{"decision":"pass"}`、`发单成功` 的伪造结果。
3. 命令执行后必须返回真实 stdout/stderr；失败就如实返回失败原因并给出下一步修复命令。
